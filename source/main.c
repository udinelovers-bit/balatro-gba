...
#define FPS (60)
#define SPLASH_DURATION (FPS * 10) // FPS * SECONDS
...
void init()
{
    ...
    REG_BG2CNT = BG_PRIO(2) | BG_CBB(AFFINE_BG_CBB) | BG_SBB(AFFINE_BG_CBB) | BG_8BPP | BG_WRAP | BG_AFF_16x16;
    ...
    for (int i = 0; i < SPLASH_DURATION; i++)
    {
        VBlankIntrWait();
        key_poll();
        ...
        tte_printf("#{P:208,144; cx:0xF000}%d", 1 + (SPLASH_DURATION - i) / FPS);
    }
    tte_erase_screen();
